...
from . import state # Access shared state (caches, map, publisher)
# --- CORRECTED IMPORT for channel name ---
from . import config # Import config to get channel names
# -----------------------------------------
...
class Agent:
    def __init__(self, agent_id: str, symbol: str, initial_strategy: str = "noise", risk_factor: float = 0.5, bankroll: float = 10000.0):
        ...
        self.risk_factor = max(0.1, min(2.0, risk_factor))
        self.bankroll = Decimal(str(bankroll))
        self.position = 0
        self.average_entry_price = Decimal("0.0")
        self.realized_pnl = Decimal("0.0")
        self.trade_count = 0
        self.total_traded_value = Decimal("0.0")
        self.last_status_publish_time = datetime.now(timezone.utc)
    ...
    # --- set_active, set_strategy, set_risk_factor, set_bankroll methods remain the same ---
    def set_active(self, active: bool):
        if self.is_active != active:
            self.is_active = active
            log.info(f"Agent {self.agent_id} activity set to: {self.is_active}")

    def set_strategy(self, strategy: str):
        if strategy in agent_strategies.STRATEGY_FUNCTIONS:
            if self.strategy_name != strategy:
                self.strategy_name = strategy
                log.info(f"Agent {self.agent_id} strategy changed to: {self.strategy_name}")
        else:
            log.warning(f"Agent {self.agent_id}: Unknown strategy '{strategy}'")

    def set_risk_factor(self, risk: float):
        old_risk = self.risk_factor
        self.risk_factor = max(0.1, min(2.0, risk))
        if old_risk != self.risk_factor:
            log.info(f"Agent {self.agent_id} risk factor changed from {old_risk:.2f} to: {self.risk_factor:.2f}")
    ...
    def set_bankroll(self, amount: float):
        try:
            ...
            if new_bankroll >= 0:
                ...
                log.info(f"Agent {self.agent_id} bankroll set to: {self.bankroll:.2f}")
            else:
                log.warning(f"Agent {self.agent_id}: Invalid negative bankroll amount {amount}")
        except (ValueError, InvalidOperation, TypeError) as e:
            log.warning(f"Agent {self.agent_id}: Could not parse bankroll amount '{amount}'. Error: {e}")
    ...
    def update_on_fill(self, filled_order_id: str, trade_price_str: str, trade_quantity: int, is_taker: bool):
        log.info(f"A:{self.agent_id}: ENTER update_on_fill for OrderID: {filled_order_id[-6:]}. "
                 f"Current State: Pos={self.position}, AvgPx={self.average_entry_price:.2f}, PNL={self.realized_pnl:.2f}, Trades={self.trade_count}")

        try:
            original_order = state.submitted_orders_map.get(filled_order_id)
            if not original_order:
                log.warning(f"A:{self.agent_id}: Order {filled_order_id} not found in submitted_orders_map during fill processing.")
                self.open_order_ids.discard(filled_order_id)
                return
            if original_order["agent_id"] != self.agent_id:
                log.debug(f"A:{self.agent_id}: Fill for other agent's order {filled_order_id[-6:]}. Discarding.")
                self.open_order_ids.discard(filled_order_id)
                return

            side = original_order["side"]
            try: price = Decimal(trade_price_str); trade_value = price * Decimal(trade_quantity)
            except (InvalidOperation, TypeError) as e: log.error(f"A:{self.agent_id}: Invalid price/qty in fill {filled_order_id}: P='{trade_price_str}', Q={trade_quantity}. Error: {e}"); return
            ...
            if old_pos == 0 and new_pos != 0: # Opening
                self.average_entry_price = price
                log.debug(f"A:{self.agent_id}: Opened new position {new_pos}. AvgPx set to {price:.2f}")
                 ...
                 log.debug(f"A:{self.agent_id}: Flattened {'Long' if old_pos > 0 else 'Short'}. PNL Calc")
                 self.realized_pnl += pnl_increment; self.average_entry_price = Decimal("0.0")
                 log.debug(f"A:{self.agent_id}: Pos Flattened. AvgPx reset. PNL Inc: {pnl_increment:.2f}")
                ...
                log.debug(f"A:{self.agent_id}: Flipped {'Long->Short' if old_pos > 0 else 'Short->Long'}. PNL Calc")
                self.realized_pnl += pnl_increment; self.average_entry_price = price
                log.debug(f"A:{self.agent_id}: Flipped position. New AvgPx set to {price:.2f}. PNL Inc: {pnl_increment:.2f}")
            elif abs(new_pos) < abs(old_pos): # Decreased
                quantity_closed = abs(Decimal(old_pos)) - abs(Decimal(new_pos))
                if old_pos > 0: pnl_increment = (price - old_avg_price) * quantity_closed
                else: pnl_increment = (old_avg_price - price) * quantity_closed
                log.debug(f"A:{self.agent_id}: Reduced {'Long' if old_pos > 0 else 'Short'} Pos. PNL Calc")
                self.realized_pnl += pnl_increment
                log.debug(f"A:{self.agent_id}: Reduced position. AvgPx remains {self.average_entry_price:.2f}. PNL Inc: {pnl_increment:.2f}")
            elif abs(new_pos) > abs(old_pos): # Increased
                if old_pos * new_pos > 0:
                     new_avg_price = (abs(Decimal(old_pos)) * old_avg_price + Decimal(trade_quantity) * price) / abs(Decimal(new_pos))
                     self.average_entry_price = new_avg_price
                     log.debug(f"A:{self.agent_id}: Increased position. New AvgPx = {new_avg_price:.4f}")
                else:
                     log.warning(f"A:{self.agent_id}: Unexpected state in position increase: old={old_pos}, new={new_pos}. Resetting avg px.")
                     self.average_entry_price = price
    ...
    # --- get_status method remains the same ---
    def get_status(self, current_bbo: Optional[Dict] = None) -> Dict[str, Any]:
        ...
        if self.position != 0 and current_bbo:
            ...
            if mark_price is None: # Fallback if only one side exists
    ...
    async def decide_and_act(self, http_client: 'httpx.AsyncClient'):
        """Agent decides orders based on strategy and market state, then submits them."""
        ...
        for i, api_response_data in enumerate(api_responses):
            if api_response_data and order_payloads[i]:
                ...
                if order_id:
                    ...
                    order_info_to_store = { "agent_id": self.agent_id, "symbol": self.symbol, "side": order_payload_submitted["side"], "quantity": order_payload_submitted["quantity"] }
                    async with state.submitted_orders_lock: state.submitted_orders_map[order_id] = order_info_to_store
                    log.info(f"A:{self.agent_id}: Stored order {order_id[-6:]} in map. Details: {order_info_to_store}")

                    if state.redis_publisher:
                        try:
                            action_message = {"timestamp": datetime.now(timezone.utc).isoformat(), "agent_id": self.agent_id, "action": "submit_order", "strategy": self.strategy_name, **order_payload_submitted, "api_order_id": order_id, "api_status": "accepted"} # Assume accepted if ID received
                            action_json = json.dumps({k: v for k, v in action_message.items() if v is not None})
                            # --- CORRECTED: Use config for channel name ---
                        ...
                        except Exception as e: log.error(f"A:{self.agent_id}: Error publishing action for order {order_id[-6:]}: {e}", exc_info=True)
    ...
    # --- cancel_all_orders placeholder remains the same ---
    # async def cancel_all_orders(self, http_client: 'httpx.AsyncClient'):
    #     pass